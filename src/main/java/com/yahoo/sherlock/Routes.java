...
public class Routes {
    ...
    public static String sendAlertToSlack(Request request, Response response) throws IOException {
        ...
        try {
            ...
            report.setMetricName("Metric: important\n" + job.getTestName());
            report.setModelName(job.getTimeseriesModel());
            report.setModelParam("3.0");
            Integer reportTime = Math.round(ZonedDateTime.now(ZoneOffset.UTC).minusHours(1).toEpochSecond() / 60);
            report.setReportQueryEndTime(reportTime);
            Integer randomDeviation = -100 + (int) (Math.random() * ((100 - (-100)) + 1));
            String anomalyTimestamp = reportTime + "@" + randomDeviation;
